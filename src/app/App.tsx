...
import { IngredientMaster, type Ingredient } from './components/IngredientMaster';
...
export default function App() {
  ...
  // Ingredient handlers
  const handleAddIngredient = (ingredient: Omit<Ingredient, 'id' | 'pricePerUnit'>) => {
  ...
  const handleUpdateIngredient = (id: string, ingredient: Omit<Ingredient, 'id' | 'pricePerUnit'>) => {
  ...
  const handleUpdateProduct = (id: string, product: Omit<Product, 'id' | 'totalCost'>) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-green-50">
      <div className="max-w-6xl mx-auto p-6">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-center mb-2">Aplikasi Manajemen Modal Produksi</h1>
          <p className="text-center text-gray-600">Kelola biaya bahan dan hitung modal produk dengan mudah</p>
        </div>
        ...
        {(ingredients.length > 0 || products.length > 0) && (
          <div className="mt-6 grid grid-cols-2 gap-4">
            <div className="bg-blue-100 rounded-lg p-4 text-center border-2 border-blue-300">
              ...
              <p className="text-2xl font-bold text-blue-700">{ingredients.length}</p>
            </div>
