...
export function ProductForm({ ingredients, product, onSave, onCancel }: ProductFormProps) {
  ...
  const handleAddIngredient = () => {
    ...
    setProductIngredients([
      ...productIngredients,
      { ingredientId: ingredients[0].id, quantity: 0, unit: ingredients[0].unit }
    ]);
  };
  ...
  const handleIngredientChange = (
    ...
    if (field === 'ingredientId') {
      // When ingredient changes, update unit to match ingredient's unit
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const invalidIngredients = productIngredients.filter(ing => ing.quantity <= 0);
  ...
  return (
    <form onSubmit={handleSubmit} className="p-6 bg-gradient-to-br from-green-50 to-blue-50 rounded-lg border-2 border-green-300 space-y-4">
        ...
        {productIngredients.length === 0 ? (
          ...
          <div className="space-y-3">
            {productIngredients.map((ing, index) => {
              const ingredient = ingredients.find(i => i.id === ing.ingredientId);
              const convertedQty = ingredient ? convertToBaseUnit(ing.quantity, ing.unit, ingredient.unit) : 0;
              const cost = ingredient ? convertedQty * ingredient.pricePerUnit : 0;
                  ...
                  {ing.quantity > 0 && ingredient && (
                    <div className="text-sm bg-green-50 p-2 rounded border border-green-200">
                      <p className="text-gray-700">
                        Estimasi biaya: <span className="font-medium text-green-700">Rp {cost.toLocaleString('id-ID')}</span>
                      </p>
                    </div>
                  )}
                </div>
      ...
      {productIngredients.length > 0 && (
        <div className="p-4 bg-green-100 rounded-lg border-2 border-green-400">
          <p className="font-medium">
            Total Estimasi Modal: <span className="text-lg text-green-700">Rp {calculateEstimatedCost().toLocaleString('id-ID')}</span>
          </p>
        </div>
      )}
